...
Several planned file imports in the Telegram linking feature use relative paths
(`../model/types`, `../api/telegram-link`) rather than the absolute `@/` alias
convention used throughout the project. The `@/*` alias maps to the project root
(`./*`). Using relative imports is inconsistent with the project's established
convention and makes files harder to move.
...
**Effort:** Trivial (find/replace during implementation — don't write relative
imports in the first place).

## Acceptance Criteria

- [ ] All new files in `features/user-profile/` use
      `@/features/user-profile/...` for intra-feature imports
- [ ] No relative `../` imports in `hooks/`, `ui/`, or `api/` files
- [ ] `npm run lint` passes

## Work Log

- 2026-05-20: Found by pattern-recognition-specialist during review of Telegram
